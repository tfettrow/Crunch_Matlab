...
elseif modality == 5 % TF HACK
    template = fullfile(spm('Dir'),'toolbox', filesep, 'suit', filesep, 'templates','SUIT.nii'); %fullfile(spm('Dir'),'toolbox\suit\templates','SUIT.nii');
    % TF HACK
    data_path = pwd;
    copyfile(template, data_path);
    movefile 'SUIT.nii' 'SUIT_1mm.nii';
    template = 'SUIT_1mm.nii';
else
...
% TF HACK (move SUIT to T1)
% matlabbatch{1}.spm.spatial.coreg.estwrite.ref = {template};
% matlabbatch{1}.spm.spatial.coreg.estwrite.source = {[deblank(vols(1,:)),',1']};%{'/Users/rorden/Desktop/3D.nii,1'};
% matlabbatch{1}.spm.spatial.coreg.estwrite.other = cellstr(vols(2:end,:));% {''};
...
% TF HACK (move SUIT to T1)
    matlabbatch{1}.spm.spatial.coreg.estimate.ref = {[deblank(vols(1,:)),',1']};;
    matlabbatch{1}.spm.spatial.coreg.estimate.source = {template}; %{'/Users/rorden/Desktop/3D.nii,1'};
