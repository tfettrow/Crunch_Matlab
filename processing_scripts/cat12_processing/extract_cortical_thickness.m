function extract_cortical_thickness(varargin)
...
for this_subject_index = 1 : length(subjects)
    subject_image = fullfile(project_path, subjects{this_subject_index}, 'Processed', 'MRI_files', '02_T1', 'CAT12_Analysis', 'surf', 's15.mesh.thickness.resampled_32k.T1.gii');
...
%     subplot(1, length(roi_network_cell), this_roi_index); hold on;
    ...
    xlim([0 length(group_names)+1])
%     ylim([0 1])
    ylabel('Avg Cortical Thickness (mm?)')
    ...
    if save_figures
        fig_title = ['thickness_' roi_name_cell{this_roi_index}];
