function plot_asl_results(varargin)
...
for this_subject_index = 1 : length(subjects)
    disp(['collecting:' subjects{this_subject_index}])
    subj_results_dir = fullfile(project_path, subjects{this_subject_index}, 'Processed', 'MRI_files', '07_ASL',results_foldername,'ANTS_Normalization');
    
    asl_results_dir = fullfile(subj_results_dir, strcat(subjects{this_subject_index},'_fsl_perfusion.txt'));
    fileID = fopen(asl_results_dir);
    ...
    asl_results_dir = fullfile(subj_results_dir, strcat(subjects{this_subject_index},'_fsl_perfusion_calib.txt'));
    fileID = fopen(asl_results_dir);
    ...
    for this_beta = 3:length(data)
        ...
        % need to write a buffer for this..
        roi_names{this_beta-2} = data{this_beta,1};
        fa_calib_results(this_subject_index,this_beta-2) = str2num(data{this_beta,2});
    end
end
...
%     set(gca,'XTick',1:length(group_names),'xticklabel',{'YA','high-OA','low-OA'},'TickLabelInterpreter','none','FontSize',16)
    ...
    if save_figures
        fig_title = strcat('group_',roi_names{this_roi_index},'_asl');
    ...
    for this_group_index = 1 : length(group_names)
        ...
        bar(this_group_subjectindices, this_group_and_roi_vol_results, 'FaceColor', group_color_matrix(this_group_index,:)); hold on;
    end
%     xlim([ 0 length(this_group_index+1)])
    title([roi_names(this_roi_index)],'interpreter','latex','FontSize',16)
%     xlim([0 length(group_names)+1])
...
%     set(gca,'XTick',1:length(group_names),'xticklabel',{'YA','high-OA','low-OA'},'TickLabelInterpreter','none','FontSize',16)
    ...
    if save_figures
        ...
        fig_title = strcat('subject_',roi_names{this_roi_index},'_asl');
        filename =  fullfile(project_path, 'figures', fig_title);
        saveas(gca, filename, 'tiff')
    end
...
for this_roi_index = 1: length(roi_names)    
    ...
    for this_group_index = 1 : length(group_names)
        this_group_subjectindices = find(group_ids==this_group_index);
        this_group_and_roi_vol_results = fa_calib_results(this_group_subjectindices,this_roi_index);
...
%     set(gca,'XTick',1:length(group_names),'xticklabel',{'YA','high-OA','low-OA'},'TickLabelInterpreter','none','FontSize',16)
    ...
    if save_figures
        fig_title = strcat('group_',roi_names{this_roi_index},'_asl_calib');
...
%     xlim([0 length(group_names)+1])
    ...
    if save_figures
        MaximizeFigureWindow
        fig_title = strcat('subject_',roi_names{this_roi_index},'_asl_calib');
...
%     writetable(fa_results_table,fullfile(project_path,'spreadsheet_data','asl_intensity_score.csv'))
% end
end