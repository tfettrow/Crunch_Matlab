...
files_to_smooth = spm_select('FPList', data_path, '^warpedToMNI_.*\.nii$');
...
for i_file = 1 : size(files_to_smooth,1)
    ...
    matlabbatch{1}.spm.spatial.smooth.prefix = 'smoothed4_';
...
for i_file = 1 : size(files_to_smooth,1)
    ...
    matlabbatch{1}.spm.spatial.smooth.prefix = 'smoothed10_';
    spm_jobman('run',matlabbatch);
    clear matlabbatch
end