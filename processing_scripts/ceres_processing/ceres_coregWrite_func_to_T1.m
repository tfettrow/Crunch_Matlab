...
for i_file = 1 : size(all_files_to_coregister,1)
    ...
    matlabbatch{1}.spm.spatial.coreg.estwrite.ref = cellstr(T1);
    matlabbatch{1}.spm.spatial.coreg.estwrite.source = cellstr(this_file_path_with_volumes(1,:)); % take the first volume
    matlabbatch{1}.spm.spatial.coreg.estwrite.other = cellstr(this_file_path_with_volumes(2:end,:));
